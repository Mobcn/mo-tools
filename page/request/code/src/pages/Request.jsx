...
import CodeEditer, { LanguageEnum } from '../components/base/CodeEditer';
import RequestHeader from '../components/test/RequestHeader';
import RequestQuery from '../components/test/RequestQuery';
 ...
 * 标签页
 */
function TabItem({ tabsValue, index, children }) {
    return (
...
function Request() {
    ...
    // 发送Headers
    let [sendHeaders, setSendHeaders] = useState(new Array());
    // 响应Headers
    let [responseHeaders, setResponseHeaders] = useState(new Array());
    // 是否在发送中
    let [loading, setLoading] = useState(false);

    // 获取请求Header参数
    const getRequestHeaders = (rows) => {
    ...
    // 获取请求Query参数
    const getRequestRequestQueries = (rows) => {
    ...
    const send = () => {
        ...
        fetch(url, options)
            .then((res) => {
                setLoading(false);
                const newResponseHeaders = new Array();
    ...
    return (
        <Box sx={{ backgroundColor: 'white' }}>
            <Stack spacing={0.25} sx={{ height: '100vh', padding: '10px', boxSizing: 'border-box' }}>
                {/* 接口地址栏 */}
